...
const getAssignedSubDepts = async function (req, res) {
  try {
    const result = await sqlHelper.transact(async (txn) => {
      const { selectedPrimary } = req.query;

      const sqlWhere = ` and subDept.primaryDeptCode = ?  `;
      const args = [selectedPrimary];
      const options = {
        top: "",
        order: "",
      };
...
const getAssetsInLocationModuleTable = async function (req, res) {
  const returnValue = await sqlHelper.transact(async (txn) => {
    ...
    const userDepartmentCode = util.currentUserToken(req).deptCode; //7011
    let args = [];

    sqlWhere = `and receivingDepartment IN (${formattedDeptCodes}) and  TransferReStatus <> 'Approved' and CondemnReStatus <> 'Approved' and categoryId<> ?`;
    args = ["30"];
    const options = {
      top: "",
...
const getAssetsByDepartment = async function (req, res) {
  const returnValue = await sqlHelper.transact(async (txn) => {
    ...
    console.log("deptCodesArray", deptCodesArray);
    // Split the single string into individual codes
    const deptCodes = deptCodesArray[0].split(",").map((code) => code.trim());
    console.log("deptCodes", deptCodes);
    // Format: '5026','5013','5050'
    const formattedDeptCodes = deptCodes.map((code) => `'${code}'`).join(",");
    console.log("formattedDeptCodes", formattedDeptCodes);
    // Final SQL (raw string)
    const sqlWhere = `AND active = 1 AND receivingDepartment IN (${formattedDeptCodes}) `;
...
const getByDeptAssetPendingCondemn = async function (req, res) {
  const returnValue = await sqlHelper.transact(async (txn) => {
    ...
    const formattedDeptCodes = deptCodes.map((code) => `'${code}'`).join(",");
    let args = [];
    sqlWhere = `and latest.rn = ? and latest.RequestedDepartment IN (${formattedDeptCodes}) `;
    args = [1];
    const options = {
      top: "",
      order: "",
...
const getAssetToTransfer = async function (req, res) {
  const returnValue = await sqlHelper.transact(async (txn) => {
    ...
    let args = [];
    sqlWhere = `and receivingDepartment IN (${formattedDeptCodes})   and transferFormNo = ? and active=?`;
    args = [transferFormNo, 0];
    const options = {
      top: "",
      order: "",
    };
...
const getCondemnListProperty = async function (req, res) {
  const returnValue = await sqlHelper.transact(async (txn) => {
    ...
    const formattedDeptCodes = deptCodes.map((code) => `'${code}'`).join(",");
    let args = [];
    sqlWhere = `and receivingDepartment IN (${formattedDeptCodes})and araForm = ? and active=?`;
    args = [araForm, 0];
    const options = {
      top: "",
      order: "",
...
const getDistinctTransferFormNo = async function (req, res) {
  const returnValue = await sqlHelper.transact(async (txn) => {
    ...
    let args = [];
    sqlWhere = `and receivingDepartment IN (${formattedDeptCodes}) and transferReStatus=? and transferFormNo IS NOT NULL AND transferFormNo <> '' `;
    console.log("sqlWheresqlWheresqlWhere", sqlWhere);
    args = [transferReStatus];
    const options = {
      top: "",
